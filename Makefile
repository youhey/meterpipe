...
.PHONY: down
down:
	@echo "🚀 Down the meterpipe App"
	docker compose down --remove-orphans
	@echo "✅ done"

.PHONY: destroy
destroy:
	@echo "🚀 Destroy the meterpipe App"
...
.PHONY: demo-seed
demo-seed:
	@echo "🧑‍🏫 seed meterpipe demo data"
	@docker compose exec -T php-cli php artisan meterpipe:demo:seed

.PHONY: sync-costs
sync-costs:
	@echo "🧑‍🏫 queue meterpipe cost sync"
	@docker compose exec -T php-cli php artisan meterpipe:sync-costs --provider=all --days=30

.PHONY: recalculate-cost-summaries
recalculate-cost-summaries:
	@echo "🧑‍🏫 recalculate meterpipe cost summaries"
	@docker compose exec -T php-cli php artisan meterpipe:recalculate-cost-summaries
