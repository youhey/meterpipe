...
class GoogleOAuthAdminTest extends TestCase
{
    ...
    {
        config(['meterpipe.admin.allowed_emails' => ['admin@example.test']]);
    ...
    public function testAllowedGoogleEmailReturnsToIntendedAdminUrl(): void
    {
        ...
        $this
            ->withSession(['url.intended' => url('/admin/cost-dashboard')])
            ->get(route('auth.google.callback'))
            ->assertRedirect(url('/admin/cost-dashboard'));
    ...
    {
        config(['meterpipe.admin.allowed_emails' => []]);
        $this->fakeGoogleUser(email: 'admin@example.test');
