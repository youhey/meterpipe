...
想定用途は、OpenAI の project / API key、Laravel Cloud の application / resource など、外部 provider の ID がどの Pipe App に対応するかを管理することです。
...
現在の cost sync では、実際の cost record への app 紐づけに `cost_dimension_mappings` を使います。
...
どちらも Pipe App への mapping ですが、現時点では用途が分かれています。

- App Integrations: 外部 provider ID と Pipe App の汎用対応表
- Cost Dimension Mappings: cost sync / cost dashboard で使う dimension 対応表

後続フェーズで App Integrations を source of truth に寄せる場合は、`cost_dimension_mappings` との重複を整理します。
